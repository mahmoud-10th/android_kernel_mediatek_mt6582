...
/* porting qualcomm protect after abnormal by lenovo wangxf14 in 20140215 */
#if 1
    char log_path[PATH_MAX];
    snprintf(log_path, PATH_MAX, "%s.fuse_log", android_data_dir.path);
    // record the mode
    // 1: normal ; 2: 0 existed 3: had other paths 4: error	
#endif    
...
/* Begin, lenovo-sw wangxf14 add for fuse factory abnormal, porting android kk modify */
    if (access(android_media_dir.path, F_OK) == -1) {
        // Ensure /data/media
        if (fs_prepare_dir(android_media_dir.path, 0770, AID_MEDIA_RW, AID_MEDIA_RW) == -1) {
            ALOGE("lenovo.sw : Failed to  Ensure /data/media");
            goto fail;
        }
    }
/* End, lenovo-sw wangxf14 add for fuse factory abnormal, porting android kk modify */

    bool is_multi_user_supported = getenv("EMULATED_STORAGE_TARGET") != NULL;
#if 0
    if (version == 0) {
#else
    if ((version == 0) ||(version == 3)){ //lenovo-sw wangxf14 add for fuse factory abnormal, porting liaohs design
#endif
...
/* End, lenovo-sw wangxf14 add for fuse factory abnormal, porting android kk modify */
        ...
        if(is_multi_user_supported){
        if (version == 0){ //lenovo-sw wangxf14 add for fuse factory abnormal, porting liaohs design
            // /data/media.tmp
...
porting qualcomm protect after abnormal by lenovo wangxf14 in 20140215
qualcomm old log:
// modified by zhongwei1@lenovo.com begin
// should check path 0 for exception issues
// but if there are others path except 0, we should handle this issues: move them into 0
*/
            // /data/media/0
            char owner_media_dir[PATH_MAX];
            snprintf(owner_media_dir, PATH_MAX, "%s0", android_media_dir.path);

        if (access(owner_media_dir, F_OK) == 0) {
            ALOGD("should be never happen: 0 is already exist, but we just avoid it \n");
            if (fs_write_atomic_int(log_path, 2) == -1) {
                ALOGE("Failed to save flag to %s: %s", log_path, strerror(errno));
                //goto fail;
            }

            struct dirent *dp;
            DIR *dir = opendir(android_media_dir.path);
            if (dir == NULL) {
                // not a directory, carry on
                goto fail;
            }
            // check the existing path in /data/media  and then  remove them into 0 path
            while ((dp = readdir(dir)) != NULL) {
                if (strcmp(dp->d_name, ".") == 0 ||
                        strcmp(dp->d_name, "..") == 0 ||
                        strcmp(dp->d_name, "0") == 0 ||
                        strcmp(dp->d_name, "obb") == 0 ||
                        strcmp(dp->d_name, "legacy") == 0)
                    continue;

                if (strlen(dp->d_name) + strlen(android_media_dir.path) + 2/*NUL and slash*/ > PATH_MAX) {
                    ALOGE("Invalid path specified: too long: %s \n", strerror(errno));
                    if (fs_write_atomic_int(log_path,4) == -1) {
                        ALOGE("Failed to save flag to %s: %s", log_path, strerror(errno));
                        goto fail;
                    }
                    continue;
                ...
                snprintf(target_dir, PATH_MAX, "%s0/%s", android_media_dir.path,dp->d_name);
                char source_dir[PATH_MAX];
                snprintf(source_dir, PATH_MAX, "%s%s", android_media_dir.path,dp->d_name);
                if (rename(source_dir, target_dir) == -1) {
                    ALOGE("Failed to move  exist path to 0 %s \n", strerror(errno));
                    if (fs_write_atomic_int(log_path, 3) == -1) {
                        ALOGE("Failed to save flag to %s: %s", log_path, strerror(errno));
                        goto fail;
                    }
                    goto fail;
                }
                if (chown(target_dir, AID_MEDIA_RW, AID_MEDIA_RW) < 0) {
                    goto fail;
                }
            }
            // Ensure the permission of existing 0 path
            if (chmod(owner_media_dir, 0770) < 0) {
                goto fail;
            }
            if (chown(owner_media_dir, AID_MEDIA_RW, AID_MEDIA_RW) < 0) {
                goto fail;
            }
        }
        // modified by zhongwei1@lenovo.com end
        else 
        { // normal  mode
            ...
            if (fs_prepare_dir(android_media_dir.path, 0770, AID_MEDIA_RW, AID_MEDIA_RW) == -1) {
                ALOGE("lenovo.sw :Failed to setup media for Create /data/media again ");
                goto fail;
            }
...
/* part Begin, lenovo-sw wangxf14 add for fuse factory abnormal, porting liaohs design */
#if 1 //lenovo.sw :
        version = 3;
        if (version != oldVersion) {
            if (fs_write_atomic_int(version_path, version) == -1) {
                ALOGE("Failed to save version 1  to %s: %s", version_path, strerror(errno));
                goto fail;
            }
        }
#endif //lenovo.sw :

/* part End, lenovo-sw wangxf14 add for fuse factory abnormal, porting liaohs design */
                ...
                while ((dirent = readdir(dir))) {
                    if (dirent->d_type == DT_DIR) {
                        ...
                        if (fs_prepare_dir(user_media_dir, 0770, AID_MEDIA_RW, AID_MEDIA_RW) == -1) {
                            ALOGE("lenovo.sw : Failed to setup user media xx 0");
                            goto fail;
                        }
                    }
                }
    ...
    if (fs_prepare_dir(media_obb_dir, 0770, AID_MEDIA_RW, AID_MEDIA_RW) == -1) {
        ALOGE("lenovo.sw : Failed to setup media obb dir 0");
        goto fail;
    }
...
fail:
    // should write back the version to avoid exception betwen 1 and 2; and avoid the instance that fs_write_atomic_int function failure
    // modified by zhongwei1@lenovo.com
