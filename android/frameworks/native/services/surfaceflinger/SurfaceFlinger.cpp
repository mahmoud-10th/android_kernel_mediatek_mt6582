...
void SurfaceFlinger::init() {
    ...
    mSFEventThread = new EventThreadEx(sfVsyncSrc);
...
double SurfaceFlinger::getFps(const sp<IBinder>& display) {
	return mFps;
}
...
/*Lenovo-sw yangyang19 add 20140320 for screenshot begin*/
status_t SurfaceFlinger::captureScreen_lenovo(const sp<IBinder>& display,
        const sp<IGraphicBufferProducer>& producer,
        uint32_t reqWidth, uint32_t reqHeight,
        uint32_t minLayerZ, uint32_t maxLayerZ) {
#ifndef MTK_DEFAULT_AOSP
    ...
    if (!producer->asBinder()->localBinder()) {
        ...
        sp<const DisplayDevice> hw(getDisplayDevice(display));
        if (hw->getSecureLayerVisible()) {
            ALOGW("[rick debug] FB is protected: PERMISSION_DENIED");
            //return PERMISSION_DENIED;
        }
    }
