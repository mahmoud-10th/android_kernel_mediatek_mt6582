...
status_t SensorService::cleanupWithoutDisableLocked(
    ...
    if (rec) {
        ...
        if (rec->removeConnection(connection)) {
            ...
            /* lenovo-sw liaoxl add 2013-10-15 start clr mLastEventSeen data buffer to fix send error event when reenable sensor */
            sensors_event_t event;
            memset(&event, 0, sizeof(event));
            mLastEventSeen.editValueFor(handle) = event;
            /* lenovo-sw liaoxl add 2013-10-15 end */
        }
        return NO_ERROR;
    }
    return BAD_VALUE;
