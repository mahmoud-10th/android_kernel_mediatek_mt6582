...
/*Lenovo-sw yangyang19 add 20140320 for screenshot begin*/
    virtual status_t captureScreen_lenovo(const sp<IBinder>& display,
            const sp<IGraphicBufferProducer>& producer,
            uint32_t reqWidth, uint32_t reqHeight,
            uint32_t minLayerZ, uint32_t maxLayerZ)
    {
        ...
        data.writeInt32(minLayerZ);
        data.writeInt32(maxLayerZ);
        remote()->transact(BnSurfaceComposer::CAPTURE_SCREEN_LENOVO, data, &reply);
        return reply.readInt32();
    }
/*Lenovo-sw yangyang19 add 20140320 for screenshot end*/
    ...
    //huangjc1 modified 2014.04.21 for FPS Debug Info begin
    virtual double getFps(const sp<IBinder>& display)
    {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
        data.writeStrongBinder(display);
        remote()->transact(BnSurfaceComposer::GET_FPS, data, &reply);
        return reply.readDouble();
    }
    //huangjc1 modified 2014.04.21 for FPS Debug Info end
};
...
/*Lenovo-sw yangyang19 add 20140320 for screenshot begin*/
        case CAPTURE_SCREEN_LENOVO: {
            ...
            uint32_t minLayerZ = data.readInt32();
            uint32_t maxLayerZ = data.readInt32();
            status_t res = captureScreen_lenovo(display, producer,
                    reqWidth, reqHeight, minLayerZ, maxLayerZ);
            reply->writeInt32(res);
            return NO_ERROR;
        }
/*Lenovo-sw yangyang19 add 20140320 for screenshot end*/
	...
	//huangjc1 added 2014.04.21 for FPS Debug Info begin
	case GET_FPS: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            DisplayInfo info;
            sp<IBinder> display = data.readStrongBinder();
            double fps = getFps(display);
            reply->writeDouble(fps);
	    return NO_ERROR;
	}
        //huangjc1 added 2014.04.21 for FPS Debug Info end
