...
status_t ScreenshotClient::update_lenovo(const sp<IBinder>& display,
        uint32_t reqWidth, uint32_t reqHeight,
        uint32_t minLayerZ, uint32_t maxLayerZ) {
    sp<ISurfaceComposer> s(ComposerService::getComposerService());
    if (s == NULL) return NO_INIT;
    ...
    status_t err = s->captureScreen_lenovo(display, mBufferQueue,
            reqWidth, reqHeight, minLayerZ, maxLayerZ);
...
status_t ScreenshotClient::update_lenovo(const sp<IBinder>& display) {
    return ScreenshotClient::update_lenovo(display, 0, 0, 0, -1UL);
}

status_t ScreenshotClient::update_lenovo(const sp<IBinder>& display,
        uint32_t reqWidth, uint32_t reqHeight) {
    return ScreenshotClient::update_lenovo(display, reqWidth, reqHeight, 0, -1UL);
}
